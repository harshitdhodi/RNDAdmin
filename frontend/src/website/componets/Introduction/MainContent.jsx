...
export default function MainContent() {
  ...
  const MenuItem = ({ title, href }) => {
    const isActive = params.pathname === href || 
                    (href === '/introduction' && params.pathname === '/')
  ...
  const breadcrumbText = params.pathname === '/vision-mission' ? 'Vision & Mission' : 'Introduction';
    ...
    <div className="min-h-screen  bg-white">
      ...
      <div className="mx-auto max-w-[75rem] px-4 py-8">
        <h1 className='text-2xl font-bold'>
          {params.slug === 'vision-mission' ? 'Vision & Mission' : 'Introduction'}
        </h1>
        ...
        <div className="lg:grid lg:grid-cols-[350px_1fr] gap-8">
          ...
          {/* Main Content */}
          {params.pathname === '/vision-mission' ? (
            <MissionVision />
          ) : (
            <AboutDescription />
          )}
