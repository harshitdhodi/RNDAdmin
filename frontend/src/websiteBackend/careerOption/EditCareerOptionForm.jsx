...
const EditCareerOptionForm = () => {
  ...
  const fetchCareer = async () => {
    try {
      const response = await axios.get(`/api/career-option/getCareeroptionById?id=${id}`, { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`/api/career-option/updateCareeroption?id=${id}`, formData, {
        headers: {
...
export default EditCareerOptionForm;
